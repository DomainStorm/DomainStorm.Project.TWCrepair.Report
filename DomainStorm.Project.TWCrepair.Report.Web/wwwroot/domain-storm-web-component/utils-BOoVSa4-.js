...
const setDelete = uncurryThis$1(setPrototype.delete);
...
const stringMatch = uncurryThis$1(stringPrototype.match);
...
//
const { all } = Promise$1;
const promiseAll = promises => apply$2(all, Promise$1, [promises]);
const promiseCatch = uncurryThis$1(promisePrototype.catch);
...
const getConstructorOf$1 = fn =>
  reflectGet(getPrototypeOf$1(fn), 'constructor');
...
const makeEnvironmentCaptor = (aGlobal, dropNames = false) => {
  ...
  freeze$3(getEnvironmentOptionsList);
...
function tameDomains(domainTaming = 'safe') {
  ...
  const globalProcess = universalThis.process || undefined;
...
/** @type {readonly [ConsoleProps, LogSeverity | undefined][]} */
const consoleMethodPermits = freeze$4([
  ...consoleLevelMethods,
  ...consoleOtherMethods,
]);
...
const makeLoggingConsoleKit = (
  ...
  const loggingConsole = fromEntries(
    arrayMap(consoleMethodPermits, ([name, _]) => {
      ...
      const method = defineName(name, (...args) => {
        arrayPush$1(logArray, [name, ...args]);
      });
      return [name, freeze$4(method)];
    }),
  );
  freeze$4(loggingConsole);

  const takeLog = () => {
    const result = freeze$4(logArray);
    logArray = [];
    return result;
  };
  freeze$4(takeLog);
  ...
  return freeze$4({ loggingConsole: typedLoggingConsole, takeLog });
};
freeze$4(makeLoggingConsoleKit);
...
const filterConsole = (baseConsole, filter, _topic = undefined) => {
  ...
  return /** @type {VirtualConsole} */ (freeze$4(filteringConsole));
};
freeze$4(filterConsole);
...
const tameConsole = (
  ...
  // Browser
  const globalWindow = universalThis.window || undefined;
  if (
    errorTrapping !== 'none' &&
...
const tameSymbolConstructor = () => {
  ...
  const SharedSymbol = functionBind(Symbol$2, undefined);
...
function setFieldValue(obj, field, value) {
    ...
    if (parseInfo.type === 'dot') {
        if (typeof obj === 'object') {
            const subObj = obj[parseInfo.firstField];
            if (subObj !== undefined) {
    ...
    if (Array.isArray(obj) && obj.length > 0 && typeof obj[0] === 'object') {
        for (let i = 0; i < obj.length; i++) {
            ...
            if (subObj && typeof subObj === 'object') {
                subObj[trimmedField] = value;
            }
        }
    }
    else if (typeof obj === 'object') {
        obj[trimmedField] = value;
    }
}
/**
 * 從給定的物件中尋找特定欄位的值。
 ...
 */
function findFieldValue(obj, parts) {
 ...
 * @returns 合併後的值的陣列
 */
function mergeFieldValue(obj, parts) {
    const result = [];
    for (const part of parts) {
        const trimmedPart = part.trim();
        if (trimmedPart) {
 ...
 * @returns 計算後的值
 */
function calculateObjectValue(obj, value, ...args) {
    if (value == null)
        return value;
    if (typeof value === 'string') {
        if (isModuleFunctionString(value)) {
            ...
            let func = obj || (typeof window !== 'undefined' ? window : globalThis);
 ...
 */
function isModuleFunctionString(code) {
 ...
 */
function evaluateConditions(value, data) {
...
function checkCondition(condition, data) {
    try {
        const field = calculateObjectValue(null, condition.field, data);
        const operator = calculateObjectValue(null, condition.operator, data);
 ...
 * @returns 合併後的類別物件
 */
function mergeClasses(value1, value2) {
 ...
 */
function resolvePropertyValue(targetObject, propertyName, functionArgs) {
 ...
 */
function appendHighlight(text, searchTerm, highlightTemplate = '<span class="bg-warning text-dark">$1</span>') {
    if (!searchTerm || !text) {
        return text;
    }
    try {
 ...
 */
function executeFunction(context, fn, ...args) {
 ...
 * : 'John' }]);
 */
function isConditionGroups(value) {
    if (value == null || !Array.isArray(value)) {
        return false;
    }
    return value.every((item) => typeof item === 'object' &&
 ...
 */
function getSelectedItemIds(items, nullable, selection = 'single') {
...
export { arrayEqual as a, getFieldValue as b, arrayMove as c, debounce as d, createFunctionFromString as e, isConditionGroups as f, generateUid as g, isConditions as h, isModuleFunctionString as i, evaluateConditions as j, escapeHtml as k, appendHighlight as l, replacePlaceholders as m, calculateObjectValue as n, mergeClasses as o, parseClassNames as p, isNonEmptyValue as q, resolvePropertyValue as r, setFieldValue as s, getSelectedItemIds as t };
//# sourceMappingURL=utils-BOoVSa4-.js.map
