...
public class RA050Service : IGetService<RA050, string>
{
    ...
    private async Task<RA050> QueryRA050(QueryRA050 condition)
    {
        ...
        if(checkAchivement != null) 
        {
            ...
            result.Performance_UnderGroundLeakageAmount.PlanAmount = checkAchivement.PlanUnderGroundLeakageAmount;
            result.Performance_UnderGroundLeakageAmount.RealAmount = checkAchivement.RealUnderGroundLeakageAmount;
            ...
            result.Performance_BenefitAmount.PlanAmount = checkAchivement.PlanBenefitAmount;
            result.Performance_BenefitAmount.RealAmount = checkAchivement.RealBenefitAmount;
            ...
            result.Expense_Personnel = new RA050_DiffAndRate(checkAchivement.PlanPersonnelExpense, checkAchivement.RealPersonnelExpense);
            result.Expense_Travel = new RA050_DiffAndRate(checkAchivement.PlanTravelExpense, checkAchivement.RealTravelExpense);
            result.Expense_Other = new RA050_DiffAndRate(checkAchivement.PlanOtherExpense, checkAchivement.RealOtherExpense);
            result.Expense_CarMaintain = new RA050_DiffAndRate(checkAchivement.PlanCarMaintainExpense, checkAchivement.RealCarMaintainExpense);
            result.Expense_ApplianceMaintain = new RA050_DiffAndRate(checkAchivement.PlanApplianceMaintainExpense, checkAchivement.RealApplianceMaintainExpense);
            result.Expense_Depreciation = new RA050_DiffAndRate(checkAchivement.PlanDepreciationExpense, checkAchivement.RealDepreciationExpense);
            result.Expense_Overtime = new RA050_DiffAndRate(checkAchivement.PlanOvertimeExpense, checkAchivement.RealOvertimeExpense);
            result.Expense_CheckOutTool = new RA050_DiffAndRate(checkAchivement.PlanCheckOutToolExpense, checkAchivement.RealCheckOutToolExpense);
            result.Expense_Fuel = new RA050_DiffAndRate(checkAchivement.PlanFuleExpense, checkAchivement.RealFuleExpense);
            result.Expense_Total = new RA050_DiffAndRate(checkAchivement.PlanTotalExpense, checkAchivement.RealTotalExpense);
            #endregion

            result.MinFlowCompare = new RA050_DiffAndRate(checkAchivement.MinFlowBefore, checkAchivement.MinFlowAfter); //從範本看, 差異值 =  檢修後 - 檢修前, 
        }
        else
        {
            result = new RA050();
